...
### **GPU クラスタ不要**
`skills_only` モードはネットワーク接続のみ必要。RL トレーニングは Tinker 互換バックエンドにオフロードされます。
...
metaclaw config rl.base_url https://mint.macaron.xin/  # 任意の backend endpoint。例: MinT
...
RL トレーニングを有効にして、ライブ会話からモデルを継続的にファインチューニングします。Tinker でも MinT でも利用できます：
...
- Tinker クラウドや MinT のような Tinker 互換バックエンドが LoRA ファインチューニングを実行し、`batch_size` サンプルごとにウェイトをホットスワップ
- 専用エボルバー LLM が失敗したエピソードから新しいスキルを抽出

Tinker クラウドを使う場合は `rl.backend` を `tinker` にするか、`auto` のままで MinT endpoint を指定しないでください。
